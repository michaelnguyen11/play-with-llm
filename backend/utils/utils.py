...
from backend.constants import HF_ACCELERATE, HF_CPU, ONNX_RUNTIME_GPU, ONNX_RUNTIME_CPU, LOGGER_NAME
...
def get_argument_parser() -> argparse.ArgumentParser:
    ...
    group.add_argument(
        ...
        type=str,
        choices=[HF_ACCELERATE, HF_CPU, ONNX_RUNTIME_GPU, ONNX_RUNTIME_CPU],
    ...
    group.add_argument(
        "--dtype", type=str, default="fp32", choices=["bf16", "fp16", "int8", "fp32"], help="dtype for model"
    )
    group.add_argument(
        "--generate_kwargs",
        type=str,
    ...
    group.add_argument("--max_batch_size", type=int,
                       help="max supported batch size")
...
def get_exception_response(query_id: int, debug: bool = False):
    ...
    response = {"error": str(e_type.__name__), "message": str(
        e_message), "query_id": query_id}
    ...
    if debug:
        ...
        for trace in trace_back:
            stack_trace.append(
                "File : {}, Line : {}, Func.Name : {}, Message : {}".format(
