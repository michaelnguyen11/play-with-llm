...
class Model:
    ...
    def post_init(self, model_name: str) -> None:
        self.is_encoder_decoder = AutoConfig.from_pretrained(
            model_name).is_encoder_decoder
    ...
    def generate(self, request: GenerateRequest) -> Union[GenerateResponse, Exception]:
        try:
            ...
            input_tokens = self.tokenizer(
                request.text, return_tensors="pt", padding=True).to(self.device)
            max_input_length_in_batch = input_tokens.input_ids[0].shape[0]

            check_max_input_length(
                max_input_length_in_batch, self.max_input_length)
    ...
    def forward(self, request: ForwardRequest) -> Union[ForwardResponse, Exception]:
        def prepare_tensors(conditioning_tokens: List[List[int]], response_tokens: List[List[int]]):
            ...
            input_ids = [conditioning_tokens[i] + response_tokens[i]
                         for i in range(bs)]
            attention_mask = [
                [1] * (len(conditioning_tokens[i]) + len(response_tokens[i])) for i in range(bs)]
            labels = [[-100] * len(conditioning_tokens[i]) +
                      response_tokens[i] for i in range(bs)]
        ...
        try:
            ...
            conditioning_tokens = self.tokenizer(
                request.conditioning_text)["input_ids"]
            response_tokens = self.tokenizer(request.response)["input_ids"]

            max_length_in_batch = max(
                [len(conditioning_tokens) + len(response_tokens)])
            check_max_input_length(max_length_in_batch, self.max_input_length)

            input_tokens = prepare_tensors(
                conditioning_tokens, response_tokens)

            for t in input_tokens:
                if torch.is_tensor(input_tokens[t]):
                    input_tokens[t] = input_tokens[t].to(self.device)

            if self.inference_type == 'ort':
                # ORTModelForCausalML doesn't support loss as output
                return ForwardResponse(is_encoder_decoder=self.is_encoder_decoder)
            else:
...
def check_max_input_length(input_token_length: int, max_input_length: int) -> None:
    ...
    if input_token_length > max_input_length:
        raise Exception(
            f"max supported input length = {max_input_length} for now")
